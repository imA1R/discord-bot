...
const commands = {
	'play': (msg) => {
		...
		if (queue[msg.guild.id].playing) return msg.channel.sendMessage('Bot zaten başka müzik çalıyor');
		let dispatcher;
		...
		(function play(song) {
			...
			msg.channel.sendMessage(` **${song.title}** adlı şarkı **${song.requester}** tarafından çalınmaya başlanıyor`);
			...
			collector.on('message', m => {
				if (m.content.startsWith(ayarlar.prefix + 'pause')) {
					...
					if (Math.round(dispatcher.volume*50) >= 100) return msg.channel.sendMessage(` ${Math.round(dispatcher.volume*50)}%`);
					dispatcher.setVolume(Math.min((dispatcher.volume*50 + (2*(m.content.split('+').length-1)))/50,2));
					msg.channel.sendMessage(` ${Math.round(dispatcher.volume*50)}%`);
				} else if (m.content.startsWith('volume-')){
					if (Math.round(dispatcher.volume*50) <= 0) return msg.channel.sendMessage(` ${Math.round(dispatcher.volume*50)}%`);
	...
	'join': (msg) => {
		return new Promise((resolve, reject) => {
			...
			if (!voiceChannel || voiceChannel.type !== 'voice') return msg.reply('Bir kanala katılmalısın ..!');
	...
	'add': async (msg) => {
		...
		try {
			...
			try {
				...
				console.log(err)
				message.channel.send('Hata meydana geldi  ≽ ' + err)
			};
		};
	...
	'queue': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`Şarkıları sıraya ekle ≽${ayarlar.prefix}add`);
		let tosend = [];
